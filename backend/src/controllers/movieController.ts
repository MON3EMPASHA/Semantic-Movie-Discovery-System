...
export const getSimilarMoviesHandler = asyncHandler(
  async (req: Request, res) => {
    ...
    if (!baseText) {
      return res.status(400).json({
        message: "Movie is missing plot description for similarity search",
      });
    }

    // Request limit + 1 results to account for the movie itself being in the results
    const embedding = await generateEmbedding(baseText);
    const results = await searchMoviesByEmbedding(embedding, limit + 1);
    const filtered = results
      .filter(({ movie: candidate }) => {
        const candidateId = candidate.id || candidate._id?.toString();
        const movieId = movie.id || movie._id?.toString();
        return candidateId !== movieId;
      })
      ...
      payload = {
        ...
        releaseYear:
          bodyData.releaseYear && bodyData.releaseYear.trim()
            ...
            : undefined,
        plot: bodyData.plot && bodyData.plot.trim() ? bodyData.plot : undefined,
        trailerUrl:
...
export const getMoviePosterHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (!movie.posterGridFSId) {
      return res
        .status(404)
        .json({ message: "Movie has no poster image stored in GridFS" });
    }

    try {
      const { buffer, contentType } = await downloadImageFromGridFS(
      ...
      return res
        .status(500)
        .json({ message: "Failed to retrieve poster image" });
    }
  }
);

// Handler for updating movie poster image
export const updateMoviePosterHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params as { id: string };
    const file = req.file;
    ...
    try {
      const movie = await updateMoviePosterImage(
        id,
        file.buffer,
        file.mimetype,
        file.originalname
